...
def run():
    solver = BruteForcePBE() if len(sys.argv) < 2 else BruteForcePBE(sys.argv[1]) if len(sys.argv) < 3 else BruteForcePBE(sys.argv[1], int(sys.argv[2]))
    solver.solve()
    solver.save()

def run_tests():
    for i in range(1, 4):
        solver = BruteForcePBE("test{}.txt".format(i)) if len(sys.argv) < 3 else BruteForcePBE("test{}.txt".format(i), int(sys.argv[2]))
        solver.solve()
