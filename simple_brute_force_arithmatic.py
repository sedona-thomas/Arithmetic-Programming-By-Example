...
'''
    Sedona Thomas
    simple_brute_force_arithmatic.py: finds solutions to the set of inputs for "x * a // b + c"
'''
...
class BruteForcePBE(object):
    ...
    def solve(self) -> None:
        self.solutions |= map(self.__format_expression, {(
            a, b, c) for a in self.scope for b in self.scope for c in self.scope if self.__is_solution(a, b, c)})
...
def run():
    solver = BruteForcePBE() if len(sys.argv) < 2 else BruteForcePBE(sys.argv[1], int(
        sys.argv[2])) if len(sys.argv) >= 3 and sys.argv[2].isdigit() else BruteForcePBE(sys.argv[1])
    solver.solve()
    solver.save()
...
def run_tests():
    for i in range(1, 4):
        solver = BruteForcePBE("test{}.txt".format(i), int(sys.argv[2])) if len(
            sys.argv) >= 3 and sys.argv[2].isdigit() else BruteForcePBE("test{}.txt".format(i))
        solver.solve()
